...
class SchemaFormItem(dbc.Row):
    def __init__(self, label, value, input_id, parent, required=False):
        ...
        field_input = self.get_field_input(
            value=value, input_id=input_id, required=required
        )
        ...
        if required:
            self.children = [
                dbc.Row(
                    [
                        dbc.Col(
                            [label, html.Span("*", style={"color": "red"})],
                            width={"size": 3},
                        ),
    ...
    def get_field_input(self, value, input_id, description=None, required=False):
        ...
        if isinstance(value, list):
            ...
            field_input = dbc.Textarea(
                id=compound_id, className="string_input", style={"font-size": "16px"}
            )

        elif "format" in value and value["format"] in ["file", "directory"]:
            compound_id["data_type"] = "path"
            input_path = dbc.Input(
                id=compound_id, className="string_input", type="input"
            )
            ...
            self.register_filebrowser_callbacks(
                modal_id=modal_id, button_id=btn_id, trigger_id=trigger_id
            )

            field_input = html.Div(
                [dbc.InputGroup([input_path, btn_open_filebrowser]), modal]
            )
            ...
            field_input = dbc.Input(
                id=compound_id, className="string_input", type=input_type, step=step
            )
    ...
    def register_filebrowser_callbacks(self, modal_id, button_id, trigger_id):
        ...
        @self.parent.container.parent_app.callback(
            ...
            [Input(button_id, "n_clicks"), Input(modal_id + "-close", "n_clicks")],
...
class SchemaForm(dbc.Card):
    ...
    def __init__(self, schema, key, container=None, parent_form=None):
        ...
        logger.info("Schema form init: {}", schema)
        self.schema = schema
        ...
        self.header = dbc.CardHeader(
            [
                dbc.Button(
                    html.H4(
                        header_text, style={"color": "black"}, className="title_" + key
                    ),
                    color="link",
                    id={
                        "type": "collapsible-toggle",
                        "container": f"{self.container.id}",
                        "index": f"{self.id}-collapsible",
                    },
                )
            ],
            style={"padding": "10px"},
        ...
        for k, v in properties.items():
            ...
            if "type" in v and (v["type"] == "array"):
                ...
                if v.get("items") is None:
                    warnings.warn(
                        f"Schema badly defined for field '{k}'. Array fields require definition of 'type'. Skipping it..."
                    )
                    continue
                ...
                if "minItems" in v:
                    ...
                    if (
                        "$ref" in v["items"]
                    ):  # search for reference somewhere else in the root schema
                    ...
                    for index in range(v["minItems"]):
                        iform = SchemaForm(
                            schema=schema, key=f"{k}-{index}", parent_form=self
                        )
                        value.append(iform)
            ...
            # If item is something not yet implemented
                ...
                warnings.warn(
                    f"Field input not yet implemented for {k}. Skipping it..."
                )
                continue
...
class SchemaFormContainer(html.Div):
    ...
    def __init__(self, id, schema, parent_app, root_path=None):
        ...
        self.children_triggers = [
            html.Div(
                id={
                    ...
                    "index": id + "-external-trigger-update-forms-values",
                },
                style={"display": "none"},
            ),
            html.Div(
                id={
                    ...
                    "index": f"{id}-external-trigger-update-links-values",
                },
                style={"display": "none"},
            ),
            html.Div(
                id=f"{id}-external-trigger-update-internal-dict",
                style={"display": "none"},
            ),
            html.Div(
                id=f"{id}-output-update-finished-verification",
                style={"display": "none"},
            ),
            html.Div(id=id + "-trigger-update-links-values", style={"display": "none"}),
            html.Div(
                id=id + "-output-placeholder-links-values", style={"display": "none"}
            ),
        ]
        ...
        self.parent_app.clientside_callback(
            ...
            """,
            Output(
                {"type": "collapsible-body", "container": f"{self.id}", "index": MATCH},
                "is_open",
            ),
            [
                Input(
        ...
        def update_internal_dict(
            ...
            for e in ids:
                ...
                if e["data_type"] == "path":
                    path_v = (
                        path_values[counter["paths"]]
                        if path_values[counter["paths"]] is not None
                        else ""
                    )
        ...
        @self.parent_app.callback(
            ...
            [
                Input(
                    {"type": "external-trigger-update-forms-values", "index": ALL},
                    "children",
                ),
                Input(
                    {
        ...
        def update_forms_values(trigger, trigger_all, *states):
            ...
            if context["type"] == "external-trigger-update-forms-values" and all(
                (trg is None) or trg == [] for trg in trigger
            ):
            ...
            for v in self.data.values():
                if v["compound_id"]["data_type"] == "path":
                    ...
                    output_tags.append(
                        [
                            {"index": i, "displayValue": e}
                            for i, e in enumerate(tags_values)
                        ]
                    )
        ...
        @self.parent_app.callback(
            ...
            [
                ...
                Input(
                    {"type": "external-trigger-update-links-values", "index": ALL},
                    "children",
                ),
            ],
            [
        ...
        def update_forms_links(trigger, trigger_all, name_change):
            ...
            if trigger_source == "external-trigger-update-links-values" and all(
                ...
                raise dash.exceptions.PreventUpdate
            if (
                trigger_source == f"{self.id}-trigger-update-links-values"
                and trigger is None
            ):
                raise dash.exceptions.PreventUpdate
            ...
            for k, v in self.data.items():
                if v["target"] is not None:
                    ...
                    options = [
                        ...
                        for v in self.data.values()
                        if (
                            v["owner_class"] == target_class
                            and "name" in v["compound_id"]["index"]
                        )
                    ]
                    if len(options) > 0:
        ...
        if "properties" in self.schema:
            for form_key, form_value in self.schema["properties"].items():
                if "renderForm" in form_value and not form_value["renderForm"]:
                    ...
                    continue
                iform = SchemaForm(schema=form_value, key=form_key, container=self)
    ...
    def data_to_nested(self):
        ...
        for k, v in self.data.items():
            ...
            if v["required"] and (
                field_value is None
                or (isinstance(field_value, str) and field_value.isspace())
                or field_value == ""
                or (str(field_value) == str(self.root_path))
            ):
        ...
        for e in dicts_list:
            ...
            output = SchemaFormContainer._create_nested_dict(
                data=e, output=output, master_key_name=master_key_name
            )
            ...
            if isinstance(v, dict):
                if isinstance(output, list) or k not in output:
                    ...
                    if isinstance(output, dict):
                        SchemaFormContainer._create_nested_dict(
                            v, output[k], master_key_name
                        )
                    elif isinstance(output, list):
                        SchemaFormContainer._create_nested_dict(
                            v, output[int(k)], master_key_name
                        )
                else:
                    SchemaFormContainer._create_nested_dict(
                        v, output[k], master_key_name
